"""
OperationModeTab - Settings tab for Standalone/Pipeline mode switching and user management

This tab controls:
1. Operation Mode: Who controls shot status (Standalone vs Pipeline)
2. User Management: Role-based permissions for notes/reviews (applies in both modes)
"""
...
class OperationModeTab(QWidget):
    """Settings tab for Operation Mode (Standalone/Pipeline) and user configuration."""
    ...
    def _setup_ui(self):
        ...
        standalone_desc = QLabel(
            "Shot Library controls shot status internally. "
            "Full control over workflow without external tools."
        )
        ...
        pipeline_desc = QLabel(
            "Shot status is controlled externally by Pipeline Control. "
            "Status changes in Shot Library are read-only."
        )
        ...
        pipeline_info_label = QLabel(
            "In Pipeline Mode:\n"
            "- Shot status badges are read-only\n"
            "- Use Pipeline Control to change shot status\n"
            "- Notes and reviews still work normally"
        )
        ...
        self._mode_btn_group.addButton(self._pipeline_radio, 1)
        ...
        # User selection group (for notes/review permissions)
        self._user_group = QGroupBox("Review User (Notes & Annotations)")
        user_layout = QVBoxLayout(self._user_group)

        user_info = QLabel(
            "Select your user profile for notes and review permissions. "
            "This applies in both Standalone and Pipeline modes."
        )
        user_info.setWordWrap(True)
        user_info.setStyleSheet("color: #888; font-size: 11px; margin-bottom: 8px;")
        ...
        # Permissions info
        self._permissions_group = QGroupBox("Your Permissions (Notes & Reviews)")
    ...
    def _load_settings(self):
        ...
        # Load operation mode from shared database
        ...
        # Load user settings from notes database
    ...
    def _update_ui_visibility(self):
        ...
        # Show pipeline info box only when pipeline mode is selected
        self._pipeline_info.setVisible(is_pipeline)

        # Always show user group and permissions (they apply for notes in both modes)
        self._user_group.setVisible(True)
        self._permissions_group.setVisible(True)
    ...
    def _update_permissions_display(self):
        ...
        # Note: Using True for is_studio_mode since we always want role-based permissions for notes
    ...
    def save_settings(self):
        ...
        # Save operation mode to shared database
        is_pipeline = self._pipeline_radio.isChecked()
        new_mode = OperationMode.PIPELINE if is_pipeline else OperationMode.STANDALONE
        
        self._control_authority.set_operation_mode(new_mode)
        
        # Save user settings to notes database
...
# Keep StudioModeTab as alias for backwards compatibility during transition
StudioModeTab = OperationModeTab


__all__ = ['OperationModeTab', 'StudioModeTab']
